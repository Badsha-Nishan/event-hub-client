...
// ⚡ সরাসরি INITIAL_EVENTS ডিক্লেয়ার করা হলো (যাতে ইম্পোর্ট এরর না আসে)
...
async function getDatabase() {
  ...
  if (count === 0) {
    ...
    console.log(
      "📦 Initial static events successfully uploaded to MongoDB Atlas."
    );
  }
...
app.post("/api/bookings", async (req: Request, res: Response) => {
  try {
    ...
    const eventQuery =
      eventId.length === 24 ? { _id: new ObjectId(eventId) } : { id: eventId };
    ...
    await eventsCollection.updateOne(eventQuery, {
      $set: { spotsLeft: newSpotsLeft },
    });
...
if (process.env.NODE_ENV !== "production") {
  app.listen(PORT, () => {
    console.log(
      `🚀 Assembly Backend actively running on: http://localhost:${PORT}`
    );
  });
}
